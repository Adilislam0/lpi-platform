...
|---|---|---|---|---|
| A-POST-01 | Full payload | `{"title":"Learn Docker","description":"Containerize apps","priority":7,"smile_phase":"reality-emulation"}` | `201` (or `200` per current implementation) | Goal object returned with generated `id`, timestamps |
| A-POST-02 | Minimal payload (title only) | `{"title":"Learn Docker"}` | `201` (or `200`) | Defaults applied (`priority=5`, `smile_phase=reality-emulation`, `description=""`) |
...
| B-GET-04 | Smile phase filter | Mixed phases created | `GET /api/v1/goals/?smile_phase=reality-emulation` | `200` | Only goals with `smile_phase="reality-emulation"` returned |
